...
from models import *
from utils import *
from utils.train_utils import random_rotate, mean_center, perform_rotation, orthonormalize_basis,normalize_caps,compute_centroids
...
import hydra
from utils.pointcloud_utils import kdtree_indexing, save_pointcloud, convert_yzx_to_xyz_basis, get_equivariant_density, rotate_density, sq_distance_mat, save_density, get_gradient_density,generate_mask
...
class Canonical_fields_trainer(pl.LightningModule):
    ...
    def forward_pass(self, batch, batch_idx, return_outputs=False, rot_mat=None):
        ...
        for iter_ in range(iter_num):
            ...
            density = batch["density"][iter_].clone()
            coords  = batch["coords"][iter_].clone()
            ...
            mask_density_list = []
            for i in range(B):
                mask_density_tensor = generate_mask(density[i])
                mask_density_list.append(mask_density_tensor)
            
            
            mask_density = torch.stack(mask_density_list).reshape(shape_).cuda()       
        ...
        for iter_ in range(1,len(loss_dictionary_list)):
            for key in loss_dictionary_list[0].keys():
                loss_dictionary_list[0][key] = loss_dictionary_list[0][key] + loss_dictionary_list[iter_][key]
    ...
    def compute_loss(self, batch, outputs, return_frame = False):
        ...
        basis_1 = torch.stack(basis_1, dim = 1) 
        ...
        orth_loss = torch.mean(torch.abs(basis_1 - orth_basis_1.detach())) #+ torch.mean(torch.abs(basis_2 - orth_basis_2.detach()))) / 2
            ...
            for i, batch in enumerate(loader):
                if i % skip == 0:
                    ...
                    for _ in range(num_rots):
                        ...
                        orth_basis_1 = (orthonormalize_basis(basis_1))
                        basis_instance_to_canonical_1 = convert_yzx_to_xyz_basis((orth_basis_1))
